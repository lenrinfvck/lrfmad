...
jQuery.fn.tabs = function(tabCon,type){
    ...
    tab.bind("change.tabs",function(e, select){
        ...
        tabCon.find(">[data-mtabcon]").removeClass("active");
        tabCon.find(">[data-mtabcon='"+select+"']").addClass("active");
    });
    return this;
};
...
jQuery.fn.testURL = function(type,stop){
    $(this).bind('click',function(event){
        window.open($(this).data('url'),type||'_self');
        if(stop){
            event.stopPropagation()
        }
    });
 ...
 */
jQuery.fn.diaAjax = function(callback){
    var dia = $(this),
...
/*
 *  opDia by lenrinfvck 2014/12/15
 *  arg.top 出现界限
 *  arg.width 按钮宽度  arg.height 按钮高度
 */
jQuery.fn.diaSet = function(ops){
    var $this = $(this),
        $input = $this.find('input');
    dialog =$('<div class="dialog diaImg" data-dialog="photo"><div class="closeBnt"></div><h4>'+ops.title+'</h4><div class="diaCon"><h4 class="closeBnt diaBottom">提交</h4></div></div>');
