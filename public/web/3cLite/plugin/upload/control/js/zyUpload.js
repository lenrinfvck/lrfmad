		...
		return this.each(function(){
			...
			this.createHtml = function(){
				...
				if(para.dragDrop){
				    ...
				    html += '							<img class="upload_image" src="../plugin/upload/control/images/add_img.png" style="width:expression(this.width > '+imgWidth+' ? '+imgWidth+'px : this.width)" />';
				    html += '						</div>';
				    html += '					</a>';
				    html += '				</div>';
					html += '			</div>';
			...
			this.funFilterEligibleFile = function(files){
				...
				for (var i = 0, file; file = files[i]; i++) {
					...
					}else if(file.type.indexOf('image/jpeg')<0&&file.type.indexOf('image/png')<0){
                        alert('请上传jpg/png');
                    }else{
						// 在这里需要判断当前所有文件中
                        arrFiles.push(file);
			...
			this.addEvent = function(){
				...
				$(".upload_btn").bind("click", function(e){
					...
					if(ZYFILE.funReturnNeedFiles().length <= 0){
                        alert("请先选中文件再点击上传");
					}else if(ZYFILE.funReturnNeedFiles().length > 5){           //上传图片限制
                        alert("最多只能上传5张照片");
                    }else{
                        $("#fileSubmit").click();
					}
